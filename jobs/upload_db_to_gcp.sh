...
docker exec ${MYSQL_CONTAINER_NAME} /usr/bin/mysqldump -u ${MYSQL_USER} --single-transaction --quick --lock-tables=false  --password=${MYSQL_ROOT_PASSWORD} covidmex_db > ${FILENAME}${DATESTAMP}.sql
...
tar czf ${FILENAME}.tar.gz ${FILENAME}${DATESTAMP}.sql
